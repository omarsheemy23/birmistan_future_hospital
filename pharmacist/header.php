...
// تحقق ما إذا كانت الجلسة مفعلة بالفعل قبل بدءها
if (session_status() == PHP_SESSION_NONE) {
    session_start();
}

// فحص إذا كان الطلب آتياً من صفحة الاختبار أو المطور
$is_test_mode = isset($_GET['test_mode']) || (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'test_pharmacy.php') !== false);

// التحقق من تسجيل دخول الصيدلي فقط إذا كانت الصفحة ليست صفحة الدخول
$current_page = basename($_SERVER['PHP_SELF']);
$allowed_pages = ['login.php', 'dashboard_fallback.php'];

if (!in_array($current_page, $allowed_pages) && (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'pharmacist')) {
